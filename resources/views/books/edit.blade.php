<x-admin-layout>
    <div x-data="{ 
            ...
            bookWriter: '{{ addslashes($book->writer) }}',
            bookPublisher: '{{ addslashes($book->publisher) }}',
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 mb-8 shadow-sm relative overflow-hidden">
            <div class="absolute -right-10 -top-10 w-64 h-64 bg-amber-50 rounded-full blur-3xl pointer-events-none"></div>
            
            <div class="relative z-10 flex flex-col md:flex-row items-end justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        <span class="w-2 h-2 bg-amber-500 rounded-full animate-pulse"></span>
                        <span class="font-mono text-xs font-bold text-slate-500 uppercase tracking-widest">Database Editor</span>
                    </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <select name="category_id" x-model="bookCategory" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all appearance-none cursor-pointer" required>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="writer" x-model="bookWriter"
                                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 transition-all" required>
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <input type="number" name="publication_year" x-model="bookYear"
                                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-mono text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 transition-all" required>
                                </div>
                                <div>
                                ...
                                <label class="block w-full group cursor-pointer">
                                    <div class="flex items-center justify-between p-4 border border-dashed border-slate-300 rounded-2xl group-hover:border-slate-900 group-hover:bg-slate-50 transition-all">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p class="text-sm font-bold text-slate-900">Change Cover</p>
                                                <p class="text-xs text-slate-500">Max 2MB (JPG, PNG, WEBP)</p>
                                        ...
                                        <span class="text-xs font-bold bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm group-hover:shadow-md transition-all">Browse</span>
                                    </div>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-24 space-y-4">
                    <div class="flex items-center justify-between px-1">
                        <span class="font-mono text-xs font-bold text-slate-400 uppercase tracking-widest">Live Preview</span>
                        <div class="flex items-center gap-2">
                            <template x-if="bookCategory">
                                <span class="px-2 py-1 bg-slate-900 text-white text-[10px] font-bold uppercase rounded tracking-wider">Modified</span>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-lg transition-all duration-500 hover:shadow-xl">
                        <div class="aspect-[3/4] bg-slate-100 relative border-b border-slate-100 overflow-hidden group">
                                ...
                                <div class="w-full h-full">
                                    ...
                                    @else
                                        <div class="w-full h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50">
                                            ...
                                            <span class="font-mono text-[10px] uppercase tracking-widest opacity-50">Current: None</span>
                                        </div>
                                    @endif
                                </div>
                            </template>
                        ...
                        <div class="p-6">
                            <h2 class="text-xl font-bold text-slate-900 leading-tight mb-1" x-text="bookTitle"></h2>
                            <p class="text-sm text-slate-500 mb-4" x-text="bookWriter"></p>
